...
config.vm.synced_folder "C:/Users/shr1l/Spring_Java_project/JavaApp-CICD", "/home/vagrant/JavaApp-CICD"
config.vm.network "forwarded_port", guest: 9292, host: 9292
  ...
  config.vm.define "web" do |web|
    ...
    web.vm.provision "shell", inline: <<-SHELL
      ...
      sudo bash -c 'cat > /etc/nginx/sites-available/spring <<\EOF
...
EOF'
  ...
  config.vm.define "db" do |db|
    ...
    db.vm.provision "shell", inline: <<-SHELL
      ...
      mysql -u root -e "CREATE DATABASE springdb;"
      mysql -u root -e "CREATE USER 'springuser'@'%' IDENTIFIED BY 'springpass';"
      mysql -u root -e "GRANT ALL PRIVILEGES ON springdb.* TO 'springuser'@'%';"
